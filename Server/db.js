...
// existing functions
async function createRoomRecord({ roomCode, status = 'lobby', maxPlayers = 4 }) { await pool.query(`INSERT INTO vervus_data.rooms (room_code, status, max_players) VALUES ($1, $2::vervus_data.room_status, $3) ON CONFLICT (room_code) DO UPDATE SET status = EXCLUDED.status`, [roomCode, status, maxPlayers]); }
...
async function updatePlayerConnection({ playerId, status }) { const isDisconnected = status !== 'connected'; await pool.query(`UPDATE vervus_data.players SET connection_status = $2, last_seen_at = now(), left_at = CASE WHEN $3 THEN now() ELSE null END WHERE id = $1::uuid`, [playerId, status, isDisconnected]); }
